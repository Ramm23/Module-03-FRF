...
if nargout == 0
    ...
    % Compensate for energy at negative frequencies (except DC & Nyquist)
    XC(2:end-1,:) = 2 * XC(2:end-1,:);
    
    % Compensate for window function and duration (N/(sum(w)/N) = 1/sum(w))
    XC = XC / sum(w);
    
    figure;
    imagesc(t,f(1:M/2+1),20 * log10(abs(XC)),[-60 0]);
    xlim([t(1) t(end)])
    ylim([f(1) fs/2])
    colorbar;
    axis xy
